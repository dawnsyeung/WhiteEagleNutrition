...
- `npm run build` now performs schema generation and stages a deployable static output in `public/`.
- Run `npm run build:schema` when you only want to refresh homepage/products/about/contact JSON-LD, product detail pages, and sitemap data.
- Product and Offer data is sourced from Shopify Storefront API product records used by the Buy Button components.
- Product detail pages at `/products/<shopify-handle>` are generated into `products/*.html` by `npm run build:schema` and include canonical metadata and Product JSON-LD.
- `sitemap.xml` is regenerated by `npm run build:schema` to include generated product detail URLs.
- If live Shopify fetch fails during schema generation, the script falls back to `scripts/structured-data/data/shopify-products-cache.json` so generation does not fail hard.
